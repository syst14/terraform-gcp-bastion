...
resource "google_storage_bucket" "default" {
  count         = "${length(var.name_bucket)}"
  name          = "${element(var.name_bucket, count.index)}"
  location      = "${length(var.location_bucket) > 0 ? var.location_bucket : data.google_client_config.default.region}"
